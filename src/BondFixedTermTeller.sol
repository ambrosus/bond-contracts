    ...
    function _handlePayout(
        ...
        if (vesting_ != 0) {
            ...
            for (uint8 i = 0; i < payout_.length; i++) {
                ...
                if (!tokenMetadata[tokenId].active) _deploy(tokenId, payoutToken_[i], expiry);
            ...
            for (uint8 i = 0; i < payout_.length; i++) payoutToken_[i].safeTransfer(recipient_, payout_[i]);
        }
    }
