...
contract BondOracle is IBondOracle, Auth {
    ...
    error Oracle_OldPrice(address token, uint256 lastUpdatedAt, uint256 currentWindow);
    // ------------  Data entry struct  ------------
    ...
    modifier onlyValidPrice(
        ...
        uint256 updatedAt = prices[tokenAddress].updatedAt;
        uint256 window = block.timestamp - VALIDITY_WINDOW;
        if (updatedAt < window) revert Oracle_OldPrice(tokenAddress, updatedAt, window);
        _;
    }

    /// @notice Set price (in USD) for smallest unit of token
    function setPrice(address token, uint256 price) external requiresAuth {
    ...
    function usdPrice(
        address tokenAddress
    ) external view onlyValidPrice(tokenAddress) returns (uint256) {
        return prices[tokenAddress].price;
    }

    /// @notice Returns the price as a ratio of quote tokens to base tokens for the provided market id scaled by
    /// 10^decimals
    ...
    /// @notice Returns the price as a ratio of quote tokens to base tokens for the provided token pair scaled by
    /// 10^decimals
    function currentPrice(
        ERC20 quoteToken_,
        ERC20 payoutToken_
    ) external view onlyValidPrice(address(quoteToken_)) onlyValidPrice(address(payoutToken_)) returns (uint256) {
        uint256 quotePrice = prices[address(quoteToken_)].price;
        uint256 payoutPrice = prices[address(payoutToken_)].price;
        uint256 quoteDecimals = quoteToken_.decimals();
        uint256 payoutDecimals = payoutToken_.decimals();

        return (quotePrice * 10 ** (payoutDecimals + oracleDecimals)) / (payoutPrice * 10 ** quoteDecimals);
    }
