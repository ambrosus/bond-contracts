...
import {IDiscountFeeder} from "../../lib/interfaces/IDiscountFeeder.sol";
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function discountedProtocolFee(
        address staker_
    ) internal view returns (uint48) {
        if (address(feeDiscountFeeder) == address(0) || address(stakingToken) == address(0)) return protocolFee;
        ...
        return uint48(
            baseFee.mulDiv(
                (DiscountHundredPercent - discountPercent),
                /// @dev 100% - discountPercent
                DiscountHundredPercent
            )
        );
        /// @dev 100%
    }
